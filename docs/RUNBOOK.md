...
| LWO アドオン | コミュニティ製 | `.lwo` `.lwob` インポート | 例: <https://github.com/nangtani/blender-import-lwo>（v1.4.7 以降が Blender 4.x 対応、配布ファイル名 `io_scene_lwo.zip`） |
| 3DS アドオン | コミュニティ製 | `.3ds` インポート | Blender 4.x ではコアから外れている。Extensions プラットフォーム |

### 0.3 参照画像

`references/ojisan/ojisan.png` （または同フォルダ内の任意の名前）。
正面気味のカット推奨。背景は単色または近い色だと perceptual hash が効きやすい。
くだん視覚比較用のキャプチャは `references/kudan/` 配下に分離。
...
提供されている参照画像（ojisan.png）の **シェーディングが滑らかすぎる**。
